...
public class ZKUtil {
  ...
  throws IOException {
    ...
    String userpassword = identifier;
    if(ensemble == null) {
    ...
    if(userpassword==null||"".equals(userpassword)){
      userpassword = conf.get("zookeeper.acl.auth");
    }
    //新增用户名密码认证
    return new RecoverableZooKeeper(ensemble, timeout, watcher,
        retry, retryIntervalMillis, userpassword);
  }
...
//    }
  ...
  /**
   * 处理安全认证的，采用安全认证，但是绕过 kerberos
   * add by 仵文辉
   * @param conf
   * @return
   */
  public static  boolean checkOther(Configuration conf){
    if("yes".equals(conf.get("hbase.security.acl"))){
      return true;
    }
    return false;
  }
  ...
  public static boolean isSecureZooKeeper(Configuration conf) {
    ...
    //add by wuwenhui 添加非kerberos安全支持
//    if(checkOther(conf)){
//      return true;
//    }
  ...
  public static ArrayList<ACL> createACL(ZooKeeperWatcher zkw, String node,
    ...
    if (isSecureZooKeeper) {
      ...
      //添加acl认证
      String superUsers = zkw.getConfiguration().get("zookeeper.acl.auth");
      if(superUsers!=null&&!"".equals(superUsers)){
//        Id ANYONE_ID_UNSAFE = new Id("auth", "zookeeper:Zookeeper#2018");
        Id ANYONE_ID_UNSAFE = new Id("auth", superUsers);
